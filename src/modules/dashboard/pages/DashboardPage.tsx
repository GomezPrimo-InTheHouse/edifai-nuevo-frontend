    ...
    <Stack spacing={3}>
      ...
      <Grid container spacing={2}>
        <Grid size={{ xs: 6, sm: 4, md: 3 }}>
          <KpiCard icon={<Building2 size={20} />}
            label={t('dashboard.kpis.obras_activas')}
            value={kpis.obras.activas}
            sub={t('dashboard.kpis.obras_total', { total: kpis.obras.total })}
            color="#F59E0B" onClick={() => navigate('/obras')} />
        </Grid>
        <Grid size={{ xs: 6, sm: 4, md: 3 }}>
          <KpiCard icon={<HardHat size={20} />}
            label={t('dashboard.kpis.labores_activas')}
            value={kpis.labores.activas}
            sub={t('dashboard.kpis.obras_total', { total: kpis.labores.total })}
            color="#3B82F6" onClick={() => navigate('/labores')} />
        </Grid>
        <Grid size={{ xs: 6, sm: 4, md: 3 }}>
          <KpiCard icon={<Users size={20} />}
            label={t('dashboard.kpis.trabajadores')}
            value={kpis.trabajadores.total}
            sub={t('dashboard.kpis.trabajadores_sub')}
            color="#10B981" onClick={() => navigate('/trabajadores')} />
        </Grid>
        <Grid size={{ xs: 6, sm: 4, md: 3 }}>
          <KpiCard icon={<Receipt size={20} />}
            label={t('dashboard.kpis.presupuestos')}
        ...
        <Grid size={{ xs: 6, sm: 4, md: 3 }}>
          <KpiCard icon={<DollarSign size={20} />}
            label={t('dashboard.kpis.pagado_periodo')}
        ...
        <Grid size={{ xs: 6, sm: 4, md: 3 }}>
          <KpiCard icon={<CalendarCheck size={20} />}
            label={t('dashboard.kpis.asistencia_hoy')}
        ...
        <Grid size={{ xs: 6, sm: 4, md: 3 }}>
          <KpiCard icon={<Package size={20} />}
            label={t('dashboard.kpis.stock_critico')}
            value={kpis.materiales_criticos}
            sub={t('dashboard.kpis.stock_sub')}
            color={kpis.materiales_criticos > 0 ? '#EF4444' : '#10B981'}
            onClick={() => navigate('/materiales')} />
        </Grid>
        <Grid size={{ xs: 6, sm: 4, md: 3 }}>
          <KpiCard icon={<LogIn size={20} />}
            label={t('dashboard.kpis.ingresos_hoy')}
            value={kpis.logins_hoy}
            sub={t('dashboard.kpis.logins_sub')}
            color="#0891B2" />
        </Grid>
      </Grid>
                    ...
                    {ausentes_hoy.map((t_item) => (
                      <Box key={`ausente-${t_item.id}`} sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        ...
                        <Box>
                          ...
                          <Typography sx={{ fontSize: 11, color: '#94A3B8' }}>{t_item.obra_nombre}</Typography>
                        </Box>
                      </Box>
                    ))}
                  </Stack>
            ...
            <CardContent sx={{ p: 2.5 }}>
              ...
              <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '4px' }}>
                {diasGrid.map((d, i) => {
                  ...
                  let bg = '#F1F5F9'; let color = '#94A3B8';
                  ...
                  {labores.slice(0, 5).map((l) => {
                    ...
                    return (
                      <Box key={l.id} onClick={() => navigate(`/labores/${l.id}`)}
                        sx={{ p: 1.5, borderRadius: 2, border: '1px solid #E2E8F0', cursor: 'pointer',
                          transition: 'all 0.15s', '&:hover': { borderColor: color, bgcolor: `${color}08` } }}>
                        <Stack direction="row" justifyContent="space-between" alignItems="center" mb={1}>
                          <Typography sx={{ fontSize: 14, fontWeight: 700, color: '#0F172A' }}>{l.nombre}</Typography>
                          <Chip label={l.estado_nombre} size="small" sx={{ fontSize: 11, fontWeight: 700, height: 22, bgcolor: `${color}18`, color }} />
                        </Stack>
                        <Stack direction="row" justifyContent="space-between" mb={0.75}>
                          <Typography sx={{ fontSize: 12, color: '#64748B' }}>{t('dashboard.worker.progreso')}</Typography>
                          <Typography sx={{ fontSize: 12, fontWeight: 700, color }}>{pct}%</Typography>
                        </Stack>
                        <LinearProgress variant="determinate" value={pct} sx={{
                  ...
                  <TableBody>
                    {ultimos_pagos.map((p) => (
                      <TableRow key={p.id} hover>
                        ...
                        <TableCell><Typography sx={{ fontSize: 13, color: '#64748B' }}>{p.forma_pago_nombre ?? '—'}</Typography></TableCell>
                        <TableCell>
                        ...
                        <TableCell><Typography sx={{ fontSize: 13, color: '#64748B' }}>{p.motivo ?? '—'}</Typography></TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
