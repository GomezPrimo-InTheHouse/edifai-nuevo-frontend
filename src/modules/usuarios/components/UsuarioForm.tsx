    ...
    <Paper sx={{ p: 3, borderRadius: 3, bgcolor: 'background.paper' }}>
      <Box component="form" onSubmit={handleSubmit((v) => onSubmit(v as unknown as UsuarioFormValues))}>
        ...
        <Typography variant="body2" fontWeight={700} sx={{ mb: 2, color: 'text.secondary' }}>
          {t('usuario_form.seccion')}
        </Typography>

        {esTrabajador && (
          <Alert severity="info" sx={{ mb: 2, borderRadius: 2 }}>
            {t('usuario_form.aviso_trabajador_pre')} <strong>{t('usuario_form.trabajador')}</strong>. {t('usuario_form.aviso_trabajador_post')} <strong>{t('usuario_form.modulo_trabajadores')}</strong>.
          </Alert>
        )}
        ...
        <Grid container spacing={2}>
          ...
          {!esTrabajador && (
            <Grid size={{ xs: 12, md: 6 }}>
              <Controller name="nombre" control={control} render={({ field }) => (
                <TextField
                  {...field} fullWidth label={t('usuario_form.nombre')}
          ...
          <Grid size={{ xs: 12, md: esTrabajador ? 12 : 6 }}>
            <Controller name="email" control={control} render={({ field }) => (
              <TextField
                {...field} fullWidth label={t('usuario_form.email')} type="email"
          ...
          {(!isEdit || esTrabajador) && (
            <Grid size={{ xs: 12, md: 6 }}>
              <Controller name="password" control={control} render={({ field }) => (
                <TextField
                  {...field} fullWidth
                  label={esTrabajador ? t('usuario_form.password_opcional') : t('usuario_form.password')}
                  type="password"
          ...
          <Grid size={{ xs: 12, md: 6 }}>
            <Controller name="rol_id" control={control} render={({ field }) => (
              <TextField
                select fullWidth label={t('usuario_form.rol')}
                value={field.value}
                onChange={(e) => field.onChange(e.target.value === '' ? '' : Number(e.target.value))}
                error={!!errors.rol_id}
                helperText={esTrabajador ? t('usuario_form.rol_bloqueado') : (errors.rol_id?.message ?? '')}
                disabled={esTrabajador}
              >
                <MenuItem value="">{t('usuario_form.seleccionar')}</MenuItem>
        ...
        <Stack direction="row" justifyContent="flex-end" sx={{ mt: 3 }}>
          <Button type="submit" variant="contained" disabled={isSubmitting}>
            {isSubmitting ? t('usuario_form.guardando') : isEdit ? t('usuario_form.actualizar') : t('usuario_form.crear')}
          </Button>
        </Stack>
      </Box>
    </Paper>
