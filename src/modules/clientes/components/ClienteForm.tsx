    ...
    <Paper sx={{ p: 3, borderRadius: 3, bgcolor: 'background.paper' }}>
      <Box component="form" onSubmit={handleSubmit((v) => onSubmit(v as ClienteFormValues))}>
        <Typography variant="body2" fontWeight={700} sx={{ mb: 2, color: 'text.secondary' }}>
          {t('clientes.form.seccion')}
        </Typography>

        <Grid container spacing={2}>
          <Grid size={{ xs: 12, md: 6 }}>
            <Controller name="nombre" control={control} render={({ field }) => (
              <TextField {...field} fullWidth label={t('clientes.form.nombre')}
                error={!!errors.nombre} helperText={errors.nombre?.message ?? ''} />
            )} />
          </Grid>
          ...
          <Grid size={{ xs: 12, md: 6 }}>
            <Controller name="apellido" control={control} render={({ field }) => (
              <TextField {...field} fullWidth label={t('clientes.form.apellido')} />
            )} />
          </Grid>

          <Grid size={{ xs: 12, md: 6 }}>
            <Controller name="razon_social" control={control} render={({ field }) => (
              <TextField {...field} fullWidth label={t('clientes.form.razon_social')} />
            )} />
          </Grid>

          <Grid size={{ xs: 12, md: 6 }}>
            <Controller name="dni_cuit" control={control} render={({ field }) => (
              <TextField {...field} fullWidth label={t('clientes.form.dni_cuit')} />
            )} />
          </Grid>

          <Grid size={{ xs: 12, md: 6 }}>
            <Controller name="telefono" control={control} render={({ field }) => (
              <TextField {...field} fullWidth label={t('clientes.form.telefono')}
                error={!!errors.telefono} helperText={errors.telefono?.message ?? ''} />
            )} />
          </Grid>
          ...
          <Grid size={{ xs: 12, md: 6 }}>
            <Controller name="email" control={control} render={({ field }) => (
              <TextField {...field} fullWidth label={t('clientes.form.email')} type="email"
                error={!!errors.email} helperText={errors.email?.message ?? ''} />
            )} />
          </Grid>
          ...
          <Grid size={{ xs: 12 }}>
            <Controller name="direccion" control={control} render={({ field }) => (
              <TextField {...field} fullWidth label={t('clientes.form.direccion')} />
            )} />
          </Grid>
        </Grid>
        ...
        <Stack direction="row" justifyContent="flex-end" sx={{ mt: 3 }}>
          <Button type="submit" variant="contained" disabled={isSubmitting}>
            {isSubmitting ? t('clientes.form.guardando') : isEdit ? t('clientes.form.actualizar') : t('clientes.form.crear')}
          </Button>
        </Stack>
      </Box>
    </Paper>
