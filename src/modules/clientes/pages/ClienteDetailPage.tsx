...
export const ClienteDetailPage: React.FC = () => {
  ...
  if (isError) return <ErrorState title="Error" message={t('clientes.error_detail')} onRetry={refetch} />;
  if (!cliente) return <ErrorState title={t('clientes.no_encontrado')} message={t('clientes.no_encontrado_msg')} />;
  ...
  return (
    <AppLayout>
      <PageHeader
        ...
        subtitle={
          <Stack direction="row" alignItems="center" gap={1}>
            <Typography variant="body2" color="text.secondary">{cliente.razon_social ?? t('clientes.cliente')}</Typography>
            <ClienteEstadoChip estadoId={cliente.estado_id} />
          </Stack>
        }
        actions={
          <Stack direction="row" spacing={1}>
            ...
            <Button variant="contained" startIcon={<Pencil size={16} />} onClick={() => navigate(`/clientes/${clienteId}/editar`)}>
              {t('clientes.acciones.editar')}
            </Button>
          </Stack>
        }
      />
      ...
      <Grid container spacing={3}>
        <Grid size={{ xs: 12, md: 8 }}>
          <Card sx={{ borderRadius: 3, bgcolor: 'background.paper', border: `1px solid ${theme.palette.divider}`, boxShadow: 'none' }}>
            <CardContent sx={{ p: 3 }}>
              <Typography variant="h6" fontWeight={700} sx={{ mb: 2 }}>{t('clientes.detail.info')}</Typography>
              <Divider sx={{ mb: 3 }} />
              <Stack spacing={2.5}>
                {cliente.telefono && <DetailRow icon={<Phone size={16} />} label={t('clientes.detail.telefono')} value={cliente.telefono} />}
                {cliente.email && <DetailRow icon={<Mail size={16} />} label={t('clientes.detail.email')} value={cliente.email} />}
                {cliente.direccion && <DetailRow icon={<MapPin size={16} />} label={t('clientes.detail.direccion')} value={cliente.direccion} />}
                {cliente.razon_social && <DetailRow icon={<Building size={16} />} label={t('clientes.detail.razon_social')} value={cliente.razon_social} />}
                {cliente.dni_cuit && (
                  <Box>
                    <Typography variant="caption" sx={{ color: 'text.secondary', fontWeight: 600, display: 'block' }}>
                      {t('clientes.detail.dni_cuit')}
                    </Typography>
                    <Chip label={cliente.dni_cuit} size="small" sx={{ bgcolor: theme.palette.action.hover, mt: 0.5 }} />
                  </Box>
                )}
              </Stack>
            </CardContent>
        ...
        <Grid size={{ xs: 12, md: 4 }}>
          <Card sx={{ borderRadius: 3, bgcolor: 'background.paper', border: `1px solid ${theme.palette.divider}`, boxShadow: 'none' }}>
            <CardContent sx={{ p: 3 }}>
              <Typography variant="h6" fontWeight={700} sx={{ mb: 2 }}>{t('clientes.detail.obras')}</Typography>
              <Divider sx={{ mb: 2 }} />
              {(!cliente.obras || cliente.obras.length === 0) ? (
                <Typography variant="body2" color="text.disabled" sx={{ textAlign: 'center', py: 2 }}>
                  {t('clientes.detail.sin_obras')}
                </Typography>
              ) : (
                <Stack spacing={1}>
                  {cliente.obras.map((obra: any) => (
