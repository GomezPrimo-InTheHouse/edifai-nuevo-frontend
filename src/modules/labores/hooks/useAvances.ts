...
export function useAvancesBySector(sector_id?: number | null) {
  return useQuery({
    queryKey: ['avances', 'sector', sector_id],
    queryFn: () => avanceApi.getBySector(sector_id!),
    enabled: Boolean(sector_id),
  });
}
...
export function useCrearAvance() {
  ...
  return useMutation({
    ...
    onSuccess: (_, vars) => {
      queryClient.invalidateQueries({ queryKey: ['avances', vars.obra_id, vars.labor_id] });
      queryClient.invalidateQueries({ queryKey: ['avances', 'sector'] });
    },
  });
}
