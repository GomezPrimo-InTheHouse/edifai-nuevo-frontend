...
import {
  ...
  FileText, Pencil, Star, User, MapPin,
} from 'lucide-react';
...
// ── Reconstruye el camino completo hasta un sector ────────────
function buildSectorPath(sectorId: number, sectores: Sector[]): Sector[] {
  const path: Sector[] = [];
...
export const LaborDetailPage: React.FC = () => {
  ...
  // Sectores de la obra — solo si hay obra_id
  const { data: sectoresObra = [] } = useSectoresPorObra(labor?.obra_id ?? 0);
  ...
  // Ruta completa del sector
  const sectorPath = labor.sector_id
    ? buildSectorPath(labor.sector_id, sectoresObra)
    : [];
