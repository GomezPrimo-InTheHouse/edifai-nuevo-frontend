...
export const LaborPresupuestosPanel: React.FC<Props> = ({ labor_id, estado_id, labor, onPresupuestoConfirmado }) => {
  ...
  // ── Card mobile por presupuesto ──────────────────────────────
  const renderCardMobile = (p: LaborPresupuesto) => {
    const enEdicion = editandoId === p.id;
    const calidadStyle = getCalidadColor(enEdicion ? editState.calidad : p.calidad);
    const nombre = p.trabajador_nombre ?? p.proveedor_nombre ?? '-';
        ...
        <Stack direction="row" alignItems="flex-start" justifyContent="space-between" mb={1.5}>
          ...
          <Stack direction="row" spacing={0.5}>
            {enEdicion ? (
              ...
              <>
                {p.estado === 'pendiente' && (
                  <IconButton size="small" onClick={() => handleIniciarEdicion(p)}>
                    <Pencil size={15} />
                  </IconButton>
                )}
                {puedeConfirmar && p.estado === 'pendiente' && (
        ...
        {enEdicion ? (
          <Stack spacing={1.5}>
            <Stack direction="row" spacing={1}>
              <TextField
                ...
                label={t('labor_presupuestos.col_precio_unitario')}
                value={editState.precio_unitario}
                onChange={(e) => setEditState((prev) => ({ ...prev, precio_unitario: e.target.value }))}
                inputProps={{ min: 0 }}
              />
              <TextField
                size="small" type="number" fullWidth
                label={t('labor_presupuestos.col_cantidad')}
                value={editState.cantidad}
                onChange={(e) => setEditState((prev) => ({ ...prev, cantidad: e.target.value }))}
                inputProps={{ min: 0 }}
              />
            </Stack>
            {precioUnitarioEdit > 0 && cantidadEdit > 0 && (
              <Typography variant="caption" color="success.main" fontWeight={700}>
                Total: ${precioTotalEdit.toLocaleString('es-AR')}
            ...
            <Stack direction="row" spacing={1}>
              <TextField
                size="small" type="number" fullWidth
                label={t('labor_presupuestos.col_plazo')}
                value={editState.plazo_dias}
                onChange={(e) => setEditState((prev) => ({ ...prev, plazo_dias: e.target.value }))}
                inputProps={{ min: 0 }}
              />
              <TextField
                ...
                label={t('labor_presupuestos.col_calidad')}
                value={editState.calidad}
                onChange={(e) => setEditState((prev) => ({ ...prev, calidad: e.target.value }))}
              >
                <MenuItem value="">-</MenuItem>
                {CALIDAD_OPTIONS.map((c) => <MenuItem key={c} value={c}>{c}</MenuItem>)}
              </TextField>
            </Stack>
            <TextField
              size="small" fullWidth multiline maxRows={2}
              label={t('labor_presupuestos.col_notas')}
              value={editState.notas}
              onChange={(e) => setEditState((prev) => ({ ...prev, notas: e.target.value }))}
            />
          </Stack>
          ...
          <>
            <Stack direction="row" spacing={1} mb={1}>
              <Box flex={1} sx={{ p: 1.5, borderRadius: 1.5, bgcolor: theme.palette.action.hover }}>
                <Typography variant="caption" color="text.secondary" display="block">
                  {t('labor_presupuestos.col_precio_unitario')}
                </Typography>
                <Typography variant="body2" fontWeight={700}>
                  ${Number(p.precio_unitario).toLocaleString('es-AR')}
                </Typography>
              ...
              <Box flex={1} sx={{ p: 1.5, borderRadius: 1.5, bgcolor: theme.palette.action.hover }}>
                <Typography variant="caption" color="text.secondary" display="block">
                  {t('labor_presupuestos.col_cantidad')}
                </Typography>
                <Typography variant="body2" fontWeight={700}>
                  {p.cantidad ? `${p.cantidad} ${unidadLabor?.simbolo ?? ''}` : '-'}
                </Typography>
            ...
            <Stack direction="row" spacing={1} mb={1}>
              <Box flex={1} sx={{ p: 1.5, borderRadius: 1.5, bgcolor: theme.palette.action.hover }}>
                <Typography variant="caption" color="text.secondary" display="block">
                  {t('labor_presupuestos.col_precio_total')}
                </Typography>
                <Typography variant="body2" fontWeight={700} color="success.main">
                  ${Number(p.precio_total).toLocaleString('es-AR')}
                </Typography>
              ...
              <Box flex={1} sx={{ p: 1.5, borderRadius: 1.5, bgcolor: theme.palette.action.hover }}>
                <Typography variant="caption" color="text.secondary" display="block">
                  {t('labor_presupuestos.col_plazo')}
                </Typography>
                <Typography variant="body2" fontWeight={700}>
                  {p.plazo_dias ? `${p.plazo_dias} días` : '-'}
                </Typography>
            ...
            {(p.calidad || p.notas) && (
              <Stack direction="row" spacing={1} alignItems="center" flexWrap="wrap">
                {p.calidad && (
                  <Chip label={p.calidad} size="small"
                    sx={{ bgcolor: calidadStyle.bg, color: calidadStyle.color, fontWeight: 700, fontSize: 11 }} />
                )}
                {p.notas && (
      ...
      <Card elevation={0} sx={{ borderRadius: 3, border: cardBorder, bgcolor: 'background.paper', mt: 3 }}>
        <CardContent sx={{ p: 3 }}>
          ...
          <Stack direction="row" alignItems="center" justifyContent="space-between" mb={2}>
            ...
            {/* Botón agregar — solo desktop, en mobile va abajo */}
            {puedeConfirmar && !haySeleccionado && !isMobile && (
              <Button variant="outlined" size="small" startIcon={<Plus size={14} />} onClick={() => setModalOpen(true)}>
                {t('labor_presupuestos.agregar')}
              </Button>
            )}
          ...
          {/* ── Desktop: tabla ── */}
          {!isLoading && presupuestos.length > 0 && !isMobile && (
            <Box sx={{ overflowX: 'auto' }}>
              <Table size="small">
                <TableHead>
                  <TableRow sx={{ bgcolor: theme.palette.action.hover }}>
          ...
          {!isLoading && presupuestos.length > 0 && isMobile && (
            <Stack spacing={2}>
              {presupuestos.map(renderCardMobile)}
            </Stack>
          )}

          {/* Botón agregar mobile — al pie */}
          {puedeConfirmar && !haySeleccionado && isMobile && (
            <Button
              variant="outlined"
              fullWidth
