...
import { Camera, CheckCircle, ChevronDown, ChevronUp, ImagePlus, MapPin, Mic, MicOff, XCircle } from 'lucide-react';
import { useAprobarAvance, useAvancesByLabor, useCrearAvance, useRechazarAvance } from '../hooks/useAvances';
import { avanceApi, type Avance } from '../../../services/api/avance.api';
import { useNotify } from '../../../shared/hooks/useNotify';
import { useAuthStore } from '../../../app/store/auth.store';
import { useSectoresPorObra } from '../../obras/hooks/useSectores';
import type { Sector } from '../../obras/types/sector.types';
...
function buildSectorPath(sectorId: number | null | undefined, sectores: Sector[]): Sector[] {
  if (!sectorId) return [];
  const path: Sector[] = [];
  let current = sectores.find(s => s.id === sectorId);
  while (current) {
    path.unshift(current);
    current = current.parent_id ? sectores.find(s => s.id === current!.parent_id) : undefined;
  }
  return path;
}
...
export const AvancesLabor: React.FC<AvancesLaborProps> = ({ obra_id, labor_id, sectorId }) => {
  const notify  = useNotify();
  const theme   = useTheme();
  const user    = useAuthStore((s) => s.user);
  const esAdmin  = ROLES_ADMIN.includes(user?.rol_id ?? 0);
  ...
  const { data: avances = [], isLoading } = useAvancesByLabor(obra_id, labor_id);
  const { data: sectoresObra = [] } = useSectoresPorObra(obra_id);
  const crearMutation = useCrearAvance();

  const sectorPath = buildSectorPath(sectorId, sectoresObra);
        ...
        {esWorker && (
          <Box sx={{ mb: 3, p: 2.5, bgcolor: theme.palette.action.hover, borderRadius: 2, border: `1px solid ${theme.palette.divider}` }}>
            ...
            <Stack spacing={2}>
              ...
              {/* Confirmación de sector */}
              {sectorPath.length > 0 && (
                <Box sx={{
                  display: 'flex', alignItems: 'center', gap: 1,
                  px: 1.5, py: 1, borderRadius: 2,
                  bgcolor: 'rgba(245,158,11,0.08)', border: '1px solid #F59E0B',
                }}>
