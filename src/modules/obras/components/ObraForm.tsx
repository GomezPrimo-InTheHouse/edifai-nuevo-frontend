...
export function ObraForm({
  ...
  // '' = sin padre | en creación: tempId del padre | en edición: String(id del padre)
    ...
    <Paper sx={{ p: 3, borderRadius: 3 }}>
      <Box component="form" onSubmit={handleSubmit(handleFormSubmit)}>
        <Grid container spacing={2}>
          ...
          <Grid size={{ xs: 12, md: 6 }}>
            <Controller name="nombre" control={control} render={({ field }) => (
              <TextField {...field} fullWidth label={t('obras.form.nombre')}
                error={!!errors.nombre} helperText={errors.nombre?.message ?? ''} />
            )} />
          </Grid>
          ...
          <Grid size={{ xs: 12, md: 6 }}>
            <Controller name="ubicacion" control={control} render={({ field }) => (
              <TextField
                {...field} fullWidth label={t('obras.form.ubicacion')}
                placeholder={t('obras.form.ubicacion_placeholder')}
                error={!!errors.ubicacion}
                helperText={
                  tieneCoords
          ...
          <Grid size={{ xs: 12 }}>
            <Controller name="descripcion" control={control} render={({ field }) => (
              <TextField {...field} fullWidth multiline minRows={3} label={t('obras.form.descripcion')}
                error={!!errors.descripcion} helperText={errors.descripcion?.message ?? ''} />
            )} />
          </Grid>
          ...
          <Grid size={{ xs: 12, md: 6 }}>
            <Controller name="tipo_obra_id" control={control} render={({ field }) => (
              <TextField select fullWidth label={t('obras.form.tipo_obra')}
                value={field.value}
                onChange={(e) => field.onChange(e.target.value === '' ? '' : Number(e.target.value))}
                error={!!errors.tipo_obra_id} helperText={errors.tipo_obra_id?.message ?? ''}>
                <MenuItem value="">{t('obras.form.seleccionar')}</MenuItem>
                {tiposObra.map((tipo) => <MenuItem key={tipo.id} value={tipo.id}>{tipo.nombre}</MenuItem>)}
              </TextField>
            )} />
          </Grid>
          ...
          <Grid size={{ xs: 12, md: 6 }}>
            <Controller name="estado_id" control={control} render={({ field }) => (
              <TextField select fullWidth label={t('obras.form.estado')}
                value={field.value}
                onChange={(e) => field.onChange(e.target.value === '' ? '' : Number(e.target.value))}
                error={!!errors.estado_id} helperText={errors.estado_id?.message ?? ''}>
                <MenuItem value="">{t('obras.form.seleccionar')}</MenuItem>
          ...
          <Grid size={{ xs: 12 }}>
            <Controller name="cliente_id" control={control} render={({ field }) => (
              <TextField select fullWidth label={t('obras.form.cliente')}
                value={field.value ?? ''}
                onChange={(e) => field.onChange(e.target.value === '' ? null : Number(e.target.value))}
                error={!!errors.cliente_id}
                helperText={errors.cliente_id?.message ?? t('obras.form.cliente_helper')}>
          ...
          <Grid size={{ xs: 12, md: 6 }}>
            <Controller name="fecha_inicio_estimado" control={control} render={({ field }) => (
              <TextField {...field} fullWidth type="date" label={t('obras.form.inicio_estimado')}
                InputLabelProps={{ shrink: true }}
                error={!!errors.fecha_inicio_estimado} helperText={errors.fecha_inicio_estimado?.message ?? ''} />
            )} />
          </Grid>
          <Grid size={{ xs: 12, md: 6 }}>
            <Controller name="fecha_fin_estimado" control={control} render={({ field }) => (
              <TextField {...field} fullWidth type="date" label={t('obras.form.fin_estimado')}
                InputLabelProps={{ shrink: true }}
                error={!!errors.fecha_fin_estimado} helperText={errors.fecha_fin_estimado?.message ?? ''} />
            )} />
          </Grid>
          ...
          <Grid size={{ xs: 12 }}>
            <Box sx={{
              ...
              <Typography variant="caption" fontWeight={700}
                sx={{ color: 'text.secondary', textTransform: 'uppercase', letterSpacing: 0.5, mb: 2, display: 'block' }}>
                Estructura de la obra
              </Typography>

              {/* Mini-form */}
              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} sx={{ mb: 2 }} alignItems="flex-start" flexWrap="wrap">
                <TextField select size="small" label="Tipo" value={nuevoTipo}
                  onChange={(e) => setNuevoTipo(e.target.value as SectorTipo)}
                  sx={{ minWidth: 120 }}>
                  {SECTOR_TIPOS_OPTIONS.map((op) => (
                    <MenuItem key={op.value} value={op.value}>{op.label}</MenuItem>
                ...
                <TextField size="small" label="Nombre / Valor" placeholder="ej: A, Norte, PB"
                  value={nuevoValor}
                  onChange={(e) => setNuevoValor(e.target.value)}
                ...
                {/* Dropdown "hijo de" */}
                <TextField select size="small" label="Hijo de"
                  ...
                  {isEditMode
                    ...
                    : parentOptionsLocal.map(({ sector, depth }) => (
                        <MenuItem key={sector.tempId} value={sector.tempId}
                          sx={{ pl: 2 + depth * 2 }}>
                          {depth > 0 ? '└ ' : ''}{nombreCompletoSector(sector)}
                        </MenuItem>
                      ))
              ...
              {isEditMode && sectoresQuery.isLoading ? (
                <Typography variant="body2" sx={{ color: 'text.disabled' }}>Cargando sectores...</Typography>
                ...
                <Stack spacing={0.5}>
                  {isEditMode
                    ? flattenedEdit.map(({ sector, depth }: { sector: Sector; depth: number }) => (
                        <Box key={sector.id} sx={{ display: 'flex', alignItems: 'center', gap: 0.5, ml: depth * 3 }}>
                          {depth > 0 && <Typography sx={{ color: 'text.disabled', fontSize: 12, lineHeight: 1 }}>└</Typography>}
                          <Chip
                    ...
                    : flattenedLocal.map(({ sector, depth }: { sector: SectorLocal; depth: number }) => (
                        <Box key={sector.tempId} sx={{ display: 'flex', alignItems: 'center', gap: 0.5, ml: depth * 3 }}>
                          {depth > 0 && <Typography sx={{ color: 'text.disabled', fontSize: 12, lineHeight: 1 }}>└</Typography>}
                          <Chip
                            label={nombreCompletoSector(sector)}
