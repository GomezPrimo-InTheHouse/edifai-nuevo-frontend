...
export const AjustePreciosModal: React.FC<AjustePreciosModalProps> = ({ open, onClose }) => {
  ...
  const handleAjuste = async () => {
    if (!porcentaje || Number(porcentaje) <= 0) {
      notify.error(t('ajuste_precios.error_porcentaje'));
      return;
    }

    const confirmed = await notify.confirm({
      title: t('ajuste_precios.confirm_title'),
      message: t('ajuste_precios.confirm_msg', {
        porcentaje,
        contexto: tipoId ? t('ajuste_precios.tipo_seleccionado') : t('ajuste_precios.todos'),
      }),
      confirmLabel: t('ajuste_precios.confirm_btn'),
      severity: 'warning',
    });
    ...
    try {
      ...
      notify.success(t('ajuste_precios.success'));
      setPorcentaje('');
      setTipoId('');
      setMotivo('');
      onClose();
    } catch (error: any) {
      const errorMsg = error.response?.data?.error || t('ajuste_precios.error_ajuste');
      notify.error(errorMsg);
    }
  };
  ...
  return (
    ...
    >
      <DialogTitle sx={{ m: 0, p: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Typography variant="h6" fontWeight={700}>{t('ajuste_precios.titulo')}</Typography>
        <IconButton onClick={onClose} size="small"><X size={18} /></IconButton>
      </DialogTitle>

      <DialogContent dividers sx={{ p: 3 }}>
        <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
          {t('ajuste_precios.descripcion')}
        </Typography>

        <Stack spacing={2.5}>
          <TextField
            fullWidth
            label={t('ajuste_precios.porcentaje')}
            type="number"
            value={porcentaje}
            onChange={(e) => setPorcentaje(e.target.value)}
            inputProps={{ min: 0.1, step: 0.1 }}
            placeholder={t('ajuste_precios.porcentaje_placeholder')}
            variant="outlined"
          />
          ...
          <TextField
            select
            fullWidth
            label={t('ajuste_precios.tipo')}
            value={tipoId}
            onChange={(e) => setTipoId(e.target.value === '' ? '' : Number(e.target.value))}
            helperText={t('ajuste_precios.tipo_helper')}
          >
            <MenuItem value="">{t('ajuste_precios.todos_materiales')}</MenuItem>
            {tipos.map((t) => (
              <MenuItem key={t.id} value={t.id}>{t.nombre}</MenuItem>
            ))}
          </TextField>
          ...
          <TextField
            fullWidth
            label={t('ajuste_precios.motivo')}
            placeholder={t('ajuste_precios.motivo_placeholder')}
            value={motivo}
            onChange={(e) => setMotivo(e.target.value)}
            multiline
            rows={2}
          ...
          <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 1.5, pt: 1 }}>
            ...
            >
              {ajusteMutation.isPending ? t('ajuste_precios.aplicando') : t('ajuste_precios.confirmar')}
            </Button>
          </Box>
        </Stack>
      </DialogContent>
