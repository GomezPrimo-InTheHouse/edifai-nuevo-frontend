...
export const PresentismoPage: React.FC = () => {
  ...
  const handleMarcar = () => {
    if (!obraId) {
      notify.warning(t('presentismo.selecciona_obra'));
      return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          notify.success(t('presentismo.marcado_ok'));
          setObraId('');
          setObservaciones('');
        } catch (err: any) {
          const msg = err?.response?.data?.message || t('presentismo.error_marcar');
          notify.error(msg);
        }
      },
      (error) => {
        ...
        switch (error.code) {
          case error.PERMISSION_DENIED:
            setGeoError(t('presentismo.geo_denegado'));
            break;
          case error.POSITION_UNAVAILABLE:
            setGeoError(t('presentismo.geo_no_disponible'));
            break;
          case error.TIMEOUT:
            setGeoError(t('presentismo.geo_timeout'));
            break;
          default:
            setGeoError(t('presentismo.geo_error'));
        }
      },
      { timeout: 10000, maximumAge: 0 }
    );
  ...
  if (loadingObras) return <AppLayout><LoadingState message={t('presentismo.cargando_obras')} /></AppLayout>;

  return (
    <AppLayout>
      <PageHeader
        title={t('presentismo.title')}
        subtitle={
          trabajador
            ? t('presentismo.subtitle_trabajador', { nombre: trabajador.nombre, apellido: trabajador.apellido })
            : t('presentismo.subtitle')
        }
      />
        ...
        <Stack spacing={3}>
              ...
              <Stack direction="row" alignItems="center" gap={1.5} mb={3}>
                ...
                <Box>
                  <Typography variant="h6" fontWeight={700} color="text.primary">
                    {t('presentismo.marcar_titulo')}
                  </Typography>
                  <Typography variant="caption" color="text.secondary">
                    {t('presentismo.marcar_desc')}
                  </Typography>
                </Box>
              </Stack>
              ...
              {obras.length === 0 ? (
                ...
                <Stack spacing={2}>
                  ...
                  <TextField
                    ...
                    label={t('presentismo.seleccionar_obra')}
                    value={obraId}
                    onChange={(e) => setObraId(Number(e.target.value))}
                    sx={{ '& .MuiOutlinedInput-root': { borderRadius: 2 } }}
                  >
                    <MenuItem value="">{t('presentismo.elegir_obra')}</MenuItem>
                    {obras.map((obra) => (
                      <MenuItem key={obra.id} value={obra.id}>
                        <Stack direction="row" alignItems="center" gap={1}>
                          <Building2 size={14} color={theme.palette.text.secondary} />
                          <Typography variant="body2">{obra.nombre}</Typography>
                          {obra.rol_en_obra && (
                            <Chip
                              label={obra.rol_en_obra}
                              size="small"
                              sx={{ fontSize: 10, height: 18, bgcolor: 'rgba(245,158,11,0.12)', color: '#B45309', fontWeight: 700 }}
                            />
                          )}
                        </Stack>
                      </MenuItem>
                    ...
                    return obra ? (
                      <Box sx={{ p: 1.5, borderRadius: 2, bgcolor: theme.palette.action.hover, border: `1px solid ${theme.palette.divider}` }}>
                        <Stack direction="row" alignItems="center" gap={1}>
                          <Building2 size={14} color={theme.palette.text.secondary} />
                          <Typography variant="caption" color="text.secondary">
                            {obra.ubicacion ?? t('presentismo.sin_direccion')}
                          </Typography>
                        </Stack>
                        {obra.fecha_hasta && (
                          <Stack direction="row" alignItems="center" gap={1} mt={0.5}>
                            ...
                            <Typography variant="caption" color="text.secondary">
                              {t('presentismo.asignado_hasta')}: {formatFecha(obra.fecha_hasta)}
                            </Typography>
                          </Stack>
                        )}
                      </Box>
                  ...
                  <TextField
                    ...
                    label={t('presentismo.observaciones')}
                    value={observaciones}
                    onChange={(e) => setObservaciones(e.target.value)}
                    multiline rows={2}
                    placeholder={t('presentismo.observaciones_placeholder')}
                    sx={{ '& .MuiOutlinedInput-root': { borderRadius: 2 } }}
                  />

                  {geoError && (
                    <Alert severity="error" onClose={() => setGeoError(null)} sx={{ borderRadius: 2 }}>
                      {geoError}
                    </Alert>
                  )}
                  ...
                  >
                    {obteniendo
                      ? t('presentismo.obteniendo_ubicacion')
                      : marcarMutation.isPending
                        ? t('presentismo.registrando')
                        : t('presentismo.confirmar')
                    }
                  </Button>

                  <Stack direction="row" alignItems="center" justifyContent="center" gap={0.5}>
                    <Navigation size={12} color={theme.palette.text.disabled} />
                    <Typography variant="caption" color="text.secondary">
                      {t('presentismo.gps_aviso')}
                    </Typography>
                  </Stack>
                </Stack>
              )}
          ...
          <Box>
            <Stack direction="row" alignItems="center" gap={1} mb={1.5}>
              ...
              <Typography variant="body2" fontWeight={700} color="text.secondary">
                {t('presentismo.historial_titulo')}
              </Typography>
            </Stack>

            {loadingHistorial ? (
              <LoadingState message={t('presentismo.cargando_historial')} />
            ) : historial.length === 0 ? (
              <Paper sx={{ p: 3, borderRadius: 3, textAlign: 'center', border: `1px solid ${theme.palette.divider}`, boxShadow: 'none', bgcolor: 'background.paper' }}>
                <Clock size={32} color={theme.palette.text.disabled} />
                <Typography variant="body2" color="text.secondary" mt={1}>
                  {t('presentismo.sin_historial')}
                </Typography>
              </Paper>
            ) : (
              <>
                ...
                <Stack spacing={1.5} sx={{ display: { xs: 'flex', md: 'none' } }}>
                  {historial.map((p) => (
                    <Paper key={p.id} sx={{ p: 2, borderRadius: 3, border: `1px solid ${theme.palette.divider}`, boxShadow: 'none', bgcolor: 'background.paper' }}>
                      <Stack direction="row" justifyContent="space-between" alignItems="flex-start">
                        <Box>
                        ...
                        <Chip
                          label={p.obra_nombre} size="small"
                          sx={{ fontSize: 11, bgcolor: 'rgba(245,158,11,0.1)', color: '#B45309', fontWeight: 600 }}
                        />
                      </Stack>
                      {p.observaciones && (
                        <>
                          <Divider sx={{ my: 1 }} />
                          <Typography variant="caption" color="text.secondary">{p.observaciones}</Typography>
                        </>
                      )}
                      <Stack direction="row" alignItems="center" gap={0.5} mt={1}>
                        <Navigation size={11} color={theme.palette.text.disabled} />
                        <Typography variant="caption" color="text.secondary">
                          {Number(p.latitud)?.toFixed(5)}, {Number(p.longitud)?.toFixed(5)}
                        </Typography>
                      </Stack>
                ...
                {/* Desktop */}
                <Paper sx={{ display: { xs: 'none', md: 'block' }, borderRadius: 3, overflow: 'hidden', border: `1px solid ${theme.palette.divider}`, boxShadow: 'none', bgcolor: 'background.paper' }}>
                  <Table size="small">
                    <TableHead sx={{ bgcolor: theme.palette.action.hover }}>
                      <TableRow>
                        <TableCell sx={{ fontWeight: 700 }}>{t('presentismo.tabla.fecha')}</TableCell>
                        <TableCell sx={{ fontWeight: 700 }}>{t('presentismo.tabla.hora')}</TableCell>
                        <TableCell sx={{ fontWeight: 700 }}>{t('presentismo.tabla.obra')}</TableCell>
                        <TableCell sx={{ fontWeight: 700 }}>{t('presentismo.tabla.coordenadas')}</TableCell>
                        <TableCell sx={{ fontWeight: 700 }}>{t('presentismo.tabla.observaciones')}</TableCell>
                      </TableRow>
                    </TableHead>
                    <TableBody>
                      {historial.map((p) => (
                        <TableRow key={p.id} hover>
                          <TableCell>
                            <Typography variant="body2" fontWeight={600}>{formatFecha(p.fecha)}</Typography>
                          </TableCell>
                          <TableCell>
                            <Typography variant="body2">{formatHora(p.fecha)}</Typography>
                          </TableCell>
                          <TableCell>
                            <Chip label={p.obra_nombre} size="small"
                              sx={{ fontSize: 11, bgcolor: 'rgba(245,158,11,0.1)', color: '#B45309', fontWeight: 600 }} />
                          </TableCell>
                          <TableCell>
                            <Stack direction="row" alignItems="center" gap={0.5}>
