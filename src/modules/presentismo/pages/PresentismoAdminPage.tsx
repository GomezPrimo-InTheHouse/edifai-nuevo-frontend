        ...
        <Box>
          ...
          {loadingStats ? <LoadingState message="Calculando estadísticas..." /> : (
            <Stack spacing={3}>
              ...
              <Grid container spacing={2}>
                <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                  <MetricCard icon={<CalendarCheck size={20} />} label="Total registros"
                    value={Number(stats?.resumen.total_registros ?? 0).toLocaleString()}
                    color="#F59E0B" subtitle={`${stats?.dias_periodo} días de período`} />
                </Grid>
                <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                  <MetricCard icon={<Users size={20} />} label="Trabajadores activos"
                    value={stats?.resumen.trabajadores_activos ?? 0}
                    color="#3B82F6" subtitle="Con al menos 1 registro" />
                </Grid>
                <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                  <MetricCard icon={<Building2 size={20} />} label="Obras con actividad"
                    value={stats?.resumen.obras_con_actividad ?? 0} color="#10B981" />
                </Grid>
                <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                  <MetricCard icon={<TrendingUp size={20} />} label="Días con actividad"
                    value={stats?.resumen.dias_con_actividad ?? 0}
                    color="#8B5CF6" subtitle={`de ${stats?.dias_periodo} días`} />
                </Grid>
              </Grid>
                        ...
                        <Stack spacing={1}>
                          {stats?.asistencia_perfecta.map((t) => (
                            <Stack key={t.id} direction="row" alignItems="center" gap={1.5}>
                              ...
                              <Chip
                                label={`${t.dias_asistidos} días`} size="small"
                                sx={{ bgcolor: 'rgba(16,185,129,0.1)', color: '#065F46', fontWeight: 700, fontSize: 11 }}
                              />
                            </Stack>
                          ))}
                  ...
                  <Card sx={{ borderRadius: 3, boxShadow: 'none', border: '1px solid #E2E8F0' }}>
                    <CardContent sx={{ p: 2.5 }}>
                      <Stack direction="row" alignItems="center" gap={1} mb={2}>
                        ...
                        <Typography variant="body2" fontWeight={700} color="text.secondary">
                          RANKING GLOBAL — PUNTOS ACUMULADOS
                        </Typography>
                      </Stack>
                      {(stats?.ranking_puntos ?? []).length === 0 ? (
                        <Typography variant="body2" color="text.secondary" textAlign="center" py={3}>
                          Ningún trabajador tiene puntos aún
                        </Typography>
                      ) : (
                        <Stack spacing={1.5}>
                          {stats?.ranking_puntos.map((t, i) => (
                            <Box key={t.id} sx={{
                              ...
                              bgcolor: i === 0 ? 'rgba(245,158,11,0.08)' : '#F8FAFC',
                              ...
                              <Stack direction="row" alignItems="center" gap={1.5}>
                                ...
                                <Box sx={{ textAlign: 'right' }}>
                                  <Typography variant="body1" fontWeight={800} color={i === 0 ? '#B45309' : '#64748B'}>
                                    {t.puntos}
                                  </Typography>
