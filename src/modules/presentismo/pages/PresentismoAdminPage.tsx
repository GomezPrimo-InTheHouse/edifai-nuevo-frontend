    ...
    <AppLayout>
                  ...
                  <Card sx={{ borderRadius: 3, boxShadow: 'none', border: `1px solid ${theme.palette.divider}`, bgcolor: 'background.paper' }}>
                    <CardContent sx={{ p: 2.5 }}>
                      ...
                      {(stats?.ranking ?? []).length === 0 ? (
                        ...
                        /* MOBILE: cards */
                        <Stack spacing={1.5}>
                          {stats?.ranking.map((tr, i) => (
                            <Box key={tr.trabajador_id} sx={{
                              p: 1.5, borderRadius: 2,
                              border: `1px solid ${theme.palette.divider}`,
                              bgcolor: theme.palette.action.hover,
                            }}>
                              <Stack direction="row" alignItems="center" gap={1.5} mb={1}>
                                <Typography sx={{ fontSize: 18, width: 24 }}>{MEDALLAS[i] ?? `${i + 1}.`}</Typography>
                                <Avatar sx={{ width: 30, height: 30, bgcolor: '#0F172A', fontSize: 11, fontWeight: 700 }}>
                                  {tr.nombre[0]}{tr.apellido[0]}
                                </Avatar>
                              ...
                              <Stack direction="row" gap={1} flexWrap="wrap">
                                <Chip
                                  label={`${tr.dias_distintos} ${t('presentismo_admin.tabla.dias')}`}
                                  size="small"
                                  sx={{ bgcolor: 'rgba(59,130,246,0.1)', color: '#1D4ED8', fontWeight: 700, fontSize: 11 }}
                                />
                                <Chip
                                  label={`${tr.total_registros} reg.`}
                                  size="small"
                                  sx={{ bgcolor: theme.palette.action.hover, fontWeight: 600, fontSize: 11 }}
                                />
                                <Chip
                                  label={`${tr.puntos ?? 0} pts`}
                                  size="small"
                                  sx={{ bgcolor: 'rgba(245,158,11,0.1)', color: '#B45309', fontWeight: 700, fontSize: 11 }}
                                />
                                {tr.ultimo_registro && (
                                  <Typography variant="caption" color="text.secondary" sx={{ alignSelf: 'center' }}>
                                    {new Date(tr.ultimo_registro).toLocaleDateString('es-AR')}
                                  </Typography>
                                )}
                              </Stack>
                            </Box>
                        ...
                        <Table size="small">
                          ...
                          <TableBody>
                            {stats?.ranking.map((tr, i) => (
                              <TableRow key={tr.trabajador_id} hover>
                                <TableCell>
                                  <Typography variant="body2" fontWeight={700}>{MEDALLAS[i] ?? i + 1}</Typography>
                                </TableCell>
                                <TableCell>
                                ...
                                <TableCell align="center">
                                  <Chip label={tr.dias_distintos} size="small"
                                    sx={{ bgcolor: 'rgba(59,130,246,0.1)', color: '#1D4ED8', fontWeight: 700, fontSize: 11 }} />
                                </TableCell>
                                <TableCell align="center">
                                  <Typography variant="body2">{tr.total_registros}</Typography>
                                </TableCell>
                                <TableCell align="center">
                                  <Chip label={`${tr.puntos ?? 0} pts`} size="small"
                                    sx={{ bgcolor: 'rgba(245,158,11,0.1)', color: '#B45309', fontWeight: 700, fontSize: 11 }} />
                                </TableCell>
                                <TableCell align="center">
                                  <Typography variant="caption" color="text.secondary">
                                    {tr.ultimo_registro ? new Date(tr.ultimo_registro).toLocaleDateString('es-AR') : '—'}
                                  </Typography>
                                </TableCell>
                              </TableRow>
                            ))}
                  ...
                  <Card sx={{ borderRadius: 3, boxShadow: 'none', border: `1px solid ${theme.palette.divider}`, bgcolor: 'background.paper', height: '100%' }}>
                    <CardContent sx={{ p: 2.5 }}>
                      ...
                      {(stats?.ausentes_hoy ?? []).length === 0 ? (
                        ...
                        <Stack spacing={1} sx={{ maxHeight: 280, overflowY: 'auto' }}>
                          {stats?.ausentes_hoy.map((ausente) => (
                            <Box key={ausente.id} sx={{ p: 1.5, borderRadius: 2, bgcolor: 'rgba(220,38,38,0.06)', border: '1px solid rgba(220,38,38,0.2)' }}>
                              <Typography variant="body2" fontWeight={600} color="#991B1B">
                                {ausente.nombre} {ausente.apellido}
                              </Typography>
                              <Typography variant="caption" color="#EF4444">{ausente.obra_nombre}</Typography>
                            </Box>
                          ))}
                        </Stack>
                      )}
                          ...
                          {stats?.ranking_puntos.map((tr, i) => (
                            <Box key={tr.id} sx={{
                              ...
                              <Stack direction="row" alignItems="center" gap={1.5}>
                                ...
                                <Typography variant="body2" fontWeight={700} flex={1}>{tr.nombre} {tr.apellido}</Typography>
                                <Box sx={{ textAlign: 'right' }}>
                                  <Typography variant="body1" fontWeight={800} color={i === 0 ? '#B45309' : 'text.secondary'}>
                                    {tr.puntos}
                                  </Typography>
      ...
      {tab === 1 && (
        <Box>
          ...
          {loadingHist ? <LoadingState message={t('presentismo_admin.cargando_historial')} /> : isMobile ? (
            /* MOBILE: cards historial */
            <Stack spacing={1.5}>
              {historial.length === 0 ? (
                <Typography color="text.disabled" textAlign="center" py={4}>
                  {t('presentismo_admin.sin_registros')}
                </Typography>
              ) : historial.map((p) => (
                <Box key={p.id} sx={{
                  p: 2, borderRadius: 2,
                  ...
                  <Typography variant="body2" color="text.secondary" mb={0.5}>{p.obra_nombre}</Typography>
