...
import { useFormasPagoList } from '../../pagos/hooks/useFormasPago';
import { useTrabajadoresList } from '../../trabajadores/hooks/useTrabajadores';
import { useObrasPorTrabajador } from '../../trabajadores/hooks/useTrabajadores';
import { useClientesList } from '../../clientes/hooks/useClientes';
import { useNotify } from '../../../shared/hooks/useNotify';
import { BotGastoImprevisto } from '../components/BotGastosImprevisto';
import type { Cliente } from '../../clientes/types/cliente.types';
...
const EMPTY_FORM: CreateGastoImprevistoPayload = {
  obra_id: 0, sector_id: null, especialidad_id: 0, descripcion: '', motivo: '',
  monto: 0, forma_pago_id: 0, pagado_por_id: undefined,
  fecha: new Date().toISOString().split('T')[0],
};
...
export const GastosImprevistosPage: React.FC = () => {
  ...
  // Obras vinculadas al trabajador logueado (vía trabajadores_obras, activas y no archivadas)
  const { data: obrasDelTrabajador = [] } = useObrasPorTrabajador(trabajadorLogueado?.id);

  const obrasParaFormulario = isWorker
    ? obras.filter((o) => obrasDelTrabajador.some((ot) => ot.id === o.id))
    : obras;
  ...
  const [editandoDeudor, setEditandoDeudor] = useState<number | null>(null);
  const [deudorSeleccionado, setDeudorSeleccionado] = useState<number | ''>('');
  const { data: sectoresDeObra = [] } = useSectoresPorObra(form.obra_id || 0);
  // ── Filtros ───────────────────────────────────────────────────
  const [filtroObra, setFiltroObra] = useState<number | ''>('');
  const [filtroEspecialidad, setFiltroEspecialidad] = useState<number | ''>('');
  const [filtroFormaPago, setFiltroFormaPago] = useState<number | ''>('');
        ...
        <>
          ...
          <Card elevation={0} sx={{
            ...
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5} flexWrap="wrap" useFlexGap>
              ...
              <TextField select fullWidth size="small" label={t('gastos.form.sector')}
  value={form.sector_id ?? ''}
  disabled={!form.obra_id || sectoresDeObra.length === 0}
  onChange={(e) => setForm(f => ({ ...f, sector_id: e.target.value ? Number(e.target.value) : null }))}>
  <MenuItem value="">{t('gastos.form.sin_sector')}</MenuItem>
        ...
        <DialogContent>
          <Stack spacing={2} sx={{ mt: 1 }}>
            ...
            <TextField select fullWidth size="small" label={t('gastos.form.obra')}
              ...
              onChange={(e) => setForm(f => ({ ...f, obra_id: Number(e.target.value) }))}>
              <MenuItem value="">{t('gastos.form.seleccionar')}</MenuItem>
              {obrasParaFormulario.map(o => <MenuItem key={o.id} value={o.id}>{o.nombre}</MenuItem>)}
            </TextField>
            <TextField select fullWidth size="small" label={t('gastos.form.especialidad')}
              error={errors.includes('especialidad_id')} value={form.especialidad_id || ''}
              onChange={(e) => setForm(f => ({ ...f, especialidad_id: Number(e.target.value) }))}>
