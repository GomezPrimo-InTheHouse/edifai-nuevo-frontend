...
export function PagoForm({
  ...
  const {
    control, handleSubmit, reset, setValue, setError, clearErrors,
    formState: { errors },
  } = useForm<PagoSchemaValues>({
    resolver: zodResolver(pagoSchema),
    defaultValues: toFormDefaults(initialData, presupuestoIdFijo, trabajadorIdFijo) as unknown as PagoSchemaValues,
  ...
  useEffect(() => {
    ...
    if (!monto || saldoPendiente === 0) return;
    if (monto > saldoPendiente) {
      setError('monto', { type: 'manual', message: t('pago_form.monto_supera', { saldo: saldoPendiente.toLocaleString('es-AR') }) });
    } else {
      clearErrors('monto');
    }
  }, [montoWatch, saldoPendiente, setError, clearErrors]);
    ...
    <Paper sx={{ p: 3, borderRadius: 3, bgcolor: 'background.paper' }}>
      <Box component="form" onSubmit={handleSubmit(handleFormSubmit)}>
        ...
        <Typography variant="body2" fontWeight={700} sx={{ mb: 1, color: 'text.secondary' }}>
          {t('pago_form.paso1_titulo')}
        </Typography>
        <Typography variant="caption" sx={{ color: 'text.disabled', display: 'block', mb: 2 }}>
          {t('pago_form.paso1_desc')}
        </Typography>

        <Grid container spacing={2} sx={{ mb: 3 }}>
          <Grid size={{ xs: 12 }}>
            <Controller name="presupuesto_id" control={control} render={({ field }) => (
              <TextField
                select fullWidth label={t('pago_form.presupuesto')}
                value={field.value}
                ...
                helperText={
                  errors.presupuesto_id?.message ??
                  (presupuestosHabilitados.length === 0 ? t('pago_form.sin_presupuestos') : '')
                }
              >
                <MenuItem value="">{t('pago_form.seleccionar_presupuesto')}</MenuItem>
                {presupuestosHabilitados.map((p) => (
                  <MenuItem key={p.id} value={p.id}>
                    {p.nombre || `${t('pagos.presupuesto')} #${p.id}`}
                  </MenuItem>
                ))}
              </TextField>
            )} />
            ...
            {isLoadingContexto ? (
              ...
              contexto && (
                <Grid container spacing={2}>
                  ...
                  {contexto.labor && (
                    <Grid size={{ xs: 12, md: contexto.cliente ? 6 : 12 }}>
                      <Box sx={{ p: 2, borderRadius: 2, bgcolor: theme.palette.action.hover, border: `1px solid ${theme.palette.divider}`, height: '100%' }}>
                        <Stack direction="row" alignItems="center" gap={1} sx={{ mb: 1 }}>
                          <Briefcase size={13} color={theme.palette.text.secondary} />
                          <Typography variant="caption" fontWeight={700} color="text.secondary">{t('pago_form.labor')}</Typography>
                        </Stack>
                        <Stack direction="row" alignItems="center" justifyContent="space-between">
                          <Typography variant="body2" fontWeight={600}>{contexto.labor.nombre ?? '-'}</Typography>
                          <Chip
                            label={contexto.labor.estado_nombre ?? t('pago_form.sin_estado')}
                            size="small"
                            sx={{ fontWeight: 600, fontSize: '0.65rem', ...laborEstadoStyles(contexto.labor.estado_nombre) }}
                          />
                        </Stack>
                        {contexto.obra && (
                          <Typography variant="caption" color="text.disabled" sx={{ mt: 0.5, display: 'block' }}>
                            {t('pago_form.obra')}: {contexto.obra.nombre}
                          </Typography>
                        )}
                      </Box>
                    </Grid>
                  ...
                  {contexto.cliente && (
                    <Grid size={{ xs: 12, md: 6 }}>
                      <Box sx={{ p: 2, borderRadius: 2, bgcolor: theme.palette.action.hover, border: `1px solid ${theme.palette.divider}`, height: '100%' }}>
                        <Stack direction="row" alignItems="center" gap={1} sx={{ mb: 1 }}>
                          <Building2 size={13} color={theme.palette.text.secondary} />
                          <Typography variant="caption" fontWeight={700} color="text.secondary">{t('pago_form.cliente')}</Typography>
                        </Stack>
                        <Typography variant="body2" fontWeight={600} sx={{ mb: 0.75 }}>
                          {contexto.cliente.nombre} {contexto.cliente.apellido}
                        </Typography>
                        <Stack spacing={0.4}>
                          {contexto.cliente.telefono && (
                            <Stack direction="row" alignItems="center" gap={0.75}>
                              <Phone size={11} color={theme.palette.text.disabled} />
                              <Typography variant="caption" color="text.secondary">{contexto.cliente.telefono}</Typography>
                            </Stack>
                          )}
                          {contexto.cliente.email && (
                            <Stack direction="row" alignItems="center" gap={0.75}>
                              <Mail size={11} color={theme.palette.text.disabled} />
                              <Typography variant="caption" color="text.secondary">{contexto.cliente.email}</Typography>
                            </Stack>
                          )}
                          {contexto.cliente.dni_cuit && (
                            <Stack direction="row" alignItems="center" gap={0.75}>
                              <CreditCard size={11} color={theme.palette.text.disabled} />
                              <Typography variant="caption" color="text.secondary">{contexto.cliente.dni_cuit}</Typography>
                            </Stack>
                          )}
                        </Stack>
                      </Box>
                  ...
                  {contexto.trabajador && (
                    <Grid size={{ xs: 12, md: contexto.equipo.length > 0 ? 4 : 12 }}>
                      <Box sx={{ p: 2, borderRadius: 2, bgcolor: theme.palette.action.hover, border: `1px solid ${theme.palette.divider}`, height: '100%' }}>
                        <Stack direction="row" alignItems="center" gap={1} sx={{ mb: 1 }}>
                          <User size={13} color={theme.palette.text.secondary} />
                          <Typography variant="caption" fontWeight={700} color="text.secondary">{t('pago_form.responsable')}</Typography>
                        </Stack>
                        <Stack direction="row" alignItems="center" gap={1.5}>
                          <Box sx={{ width: 34, height: 34, borderRadius: '50%', bgcolor: '#0F172A', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                            <Typography variant="caption" fontWeight={700} sx={{ color: '#fff' }}>
                          ...
                          <Box>
                            ...
                            <Typography variant="caption" color="text.disabled">
                              {contexto.trabajador.especialidad ?? t('pago_form.sin_especialidad')}
                            </Typography>
                          </Box>
                        </Stack>
                      </Box>
                      ...
                      <Box sx={{ p: 2, borderRadius: 2, bgcolor: theme.palette.action.hover, border: `1px solid ${theme.palette.divider}`, height: '100%' }}>
                        <Stack direction="row" alignItems="center" gap={1} sx={{ mb: 1 }}>
                          ...
                          <Typography variant="caption" fontWeight={700} color="text.secondary">
                            {t('pago_form.equipo')} ({contexto.equipo.length})
                          </Typography>
                        </Stack>
                        <Stack spacing={0.75} sx={{ maxHeight: 130, overflowY: 'auto' }}>
                          {contexto.equipo.map((m: any) => (
                            <Stack key={m.id} direction="row" alignItems="center" gap={1}>
                              <Box sx={{ width: 26, height: 26, borderRadius: '50%', bgcolor: theme.palette.divider, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                <Typography variant="caption" fontWeight={700} sx={{ color: theme.palette.text.secondary, fontSize: '0.6rem' }}>
                                  {m.nombre[0]}{m.apellido?.[0]}
                                </Typography>
                              </Box>
                              <Box>
                                ...
                                <Typography variant="caption" color="text.disabled">{m.especialidad ?? '-'}</Typography>
                              </Box>
                            </Stack>
                          ))}
                        </Stack>
                  ...
                  {contexto.materiales.length > 0 && (
                    <Grid size={{ xs: 12 }}>
                      <Box sx={{ p: 2, borderRadius: 2, bgcolor: theme.palette.action.hover, border: `1px solid ${theme.palette.divider}` }}>
                        <Stack direction="row" alignItems="center" gap={1} sx={{ mb: 1 }}>
                          <Package size={13} color={theme.palette.text.secondary} />
                          <Typography variant="caption" fontWeight={700} color="text.secondary">
                            {t('pago_form.materiales')} ({contexto.materiales.length})
                          </Typography>
                        </Stack>
                        <Stack spacing={0.5} sx={{ maxHeight: 160, overflowY: 'auto' }}>
                          {contexto.materiales.map((m: any) => (
                            <Stack
                              ...
                              sx={{ p: 1, borderRadius: 1.5, bgcolor: 'background.paper', border: `1px solid ${theme.palette.divider}` }}
                            >
                              <Typography variant="body2" fontWeight={500}>{m.material_nombre}</Typography>
                              <Stack direction="row" alignItems="center" gap={2}>
                                <Typography variant="caption" color="text.secondary">{m.cantidad} {m.unidad}</Typography>
                                <Typography variant="body2" fontWeight={600}>${Number(m.subtotal).toLocaleString('es-AR')}</Typography>
                              </Stack>
                            </Stack>
                          ))}
        ...
        {Number(trabajadorIdWatch) > 0 && (
          <Box sx={{ mb: 3 }}>
            <Typography variant="body2" fontWeight={700} sx={{ mb: 2, color: 'text.secondary' }}>
              {t('pago_form.situacion_financiera')}
            </Typography>
            ...
            {isLoadingHistorial ? (
              ...
              <>
                {presupuestoCancelado && (
                  <Alert icon={<CheckCircle2 size={18} />} severity="success" sx={{ mb: 2, borderRadius: 2, fontWeight: 600 }}>
                    {t('pago_form.cancelado_total')}
                  </Alert>
                )}
                ...
                <Grid container spacing={2} sx={{ mb: 2 }}>
                  <Grid size={{ xs: 6, md: 3 }}>
                    <Box sx={{ p: 1.5, borderRadius: 2, bgcolor: '#DCFCE7', textAlign: 'center' }}>
                      <Typography variant="caption" sx={{ color: '#166534', display: 'block', fontWeight: 600 }}>{t('pago_form.pagado')}</Typography>
                      <Typography variant="body1" fontWeight={800} sx={{ color: '#15803D' }}>
                        ${(resumen?.total_pagado ?? 0).toLocaleString('es-AR')}
                      </Typography>
                    </Box>
                  ...
                  <Grid size={{ xs: 6, md: 3 }}>
                    <Box sx={{ p: 1.5, borderRadius: 2, bgcolor: '#FEF9C3', textAlign: 'center' }}>
                      <Typography variant="caption" sx={{ color: '#854D0E', display: 'block', fontWeight: 600 }}>{t('pago_form.parcial')}</Typography>
                      <Typography variant="body1" fontWeight={800} sx={{ color: '#A16207' }}>
                        ${(resumen?.total_parcial ?? 0).toLocaleString('es-AR')}
                      </Typography>
                    </Box>
                  ...
                  <Grid size={{ xs: 6, md: 3 }}>
                    <Box sx={{ p: 1.5, borderRadius: 2, textAlign: 'center', bgcolor: saldoPendiente === 0 ? theme.palette.action.hover : '#FEE2E2' }}>
                      <Typography variant="caption" sx={{ color: saldoPendiente === 0 ? theme.palette.text.secondary : '#991B1B', display: 'block', fontWeight: 600 }}>
                        {t('pago_form.saldo_pendiente')}
                      </Typography>
                      <Typography variant="body1" fontWeight={800} sx={{ color: saldoPendiente === 0 ? theme.palette.text.secondary : '#DC2626' }}>
                        ${saldoPendiente.toLocaleString('es-AR')}
                      </Typography>
                    </Box>
                  </Grid>
                  <Grid size={{ xs: 6, md: 3 }}>
                    <Box sx={{ p: 1.5, borderRadius: 2, bgcolor: '#0F172A', textAlign: 'center' }}>
                      <Typography variant="caption" sx={{ color: '#94A3B8', display: 'block', fontWeight: 600 }}>{t('pago_form.cobrado')}</Typography>
                      <Typography variant="body1" fontWeight={800} sx={{ color: '#FFFFFF' }}>
                        ${(resumen?.total_cobrado ?? 0).toLocaleString('es-AR')}
                      </Typography>
                    </Box>
                ...
                <Typography variant="caption" fontWeight={600} sx={{ color: 'text.secondary', display: 'block', mb: 1.5 }}>
                  {t('pago_form.ultimos_pagos')} ({historial.length})
                </Typography>

                {historial.length === 0 ? (
                  <Typography variant="body2" color="text.disabled" sx={{ textAlign: 'center', py: 1 }}>
                    {t('pago_form.sin_pagos_registrados')}
                  </Typography>
                ) : (
                  <Stack spacing={1} sx={{ maxHeight: 240, overflowY: 'auto' }}>
                    {historial.map((p) => (
                      <Box key={p.id} sx={{
                        ...
                        <Box>
                          <Typography variant="body2" fontWeight={600} sx={{ lineHeight: 1.2 }}>
                            {p.presupuesto_nombre ?? `${t('pagos.presupuesto')} #${p.presupuesto_id}`}
                          </Typography>
                          <Typography variant="caption" color="text.disabled">
                            {new Date(p.fecha).toLocaleDateString('es-AR')} · {p.forma_pago_nombre ?? t('pago_form.sin_forma_pago')}
                          </Typography>
                        </Box>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          ...
                          <Chip label={p.estado} size="small" sx={{ fontWeight: 600, fontSize: '0.65rem', ...estadoChipStyles(p.estado) }} />
                        </Box>
                      </Box>
                    ))}
                  </Stack>
        ...
        {!presupuestoCancelado && Number(presupuestoIdWatch) > 0 && (
          <>
            <Divider sx={{ mb: 3 }} />
            <Typography variant="body2" fontWeight={700} sx={{ mb: 1, color: 'text.secondary' }}>
              {t('pago_form.paso2_titulo')}
            </Typography>
            {saldoPendiente > 0 && (
              <Typography variant="caption" color="text.disabled" sx={{ display: 'block', mb: 2 }}>
                {t('pago_form.saldo_disponible')}{' '}
                <strong style={{ color: '#DC2626' }}>${saldoPendiente.toLocaleString('es-AR')}</strong>
              </Typography>
            )}
            ...
            <Grid container spacing={2}>
              <Grid size={{ xs: 12, md: 4 }}>
                <Controller name="monto" control={control} render={({ field }) => (
                  <TextField
                    {...field} fullWidth type="number" label={t('pago_form.monto')}
                    onChange={(e) => field.onChange(e.target.value === '' ? '' : Number(e.target.value))}
                    inputProps={{ max: saldoPendiente > 0 ? saldoPendiente : undefined, min: 1 }}
                    InputProps={{ startAdornment: <InputAdornment position="start">$</InputAdornment> }}
                    error={!!errors.monto}
                    helperText={errors.monto?.message ?? (saldoPendiente > 0 ? t('pago_form.maximo', { saldo: saldoPendiente.toLocaleString('es-AR') }) : '')}
                  />
                )} />
              </Grid>
              ...
              <Grid size={{ xs: 12, md: 4 }}>
                <Controller name="fecha" control={control} render={({ field }) => (
                  <TextField
                    {...field} fullWidth type="date" label={t('pago_form.fecha')}
                    InputLabelProps={{ shrink: true }}
              ...
              <Grid size={{ xs: 12, md: 4 }}>
                <Controller name="estado" control={control} render={({ field }) => (
                  <TextField
                    select fullWidth label={t('pago_form.estado')}
              ...
              <Grid size={{ xs: 12, md: 6 }}>
                <Controller name="forma_pago_id" control={control} render={({ field }) => (
                  <TextField
                    select fullWidth label={t('pago_form.forma_pago')}
                    value={field.value}
                    onChange={(e) => field.onChange(e.target.value === '' ? '' : Number(e.target.value))}
                  >
                    <MenuItem value="">{t('pago_form.sin_especificar')}</MenuItem>
              ...
              <Grid size={{ xs: 12, md: 6 }}>
                <Controller name="motivo" control={control} render={({ field }) => (
                  <TextField {...field} fullWidth label={t('pago_form.motivo')} />
                )} />
              </Grid>
            </Grid>
            ...
            <Stack direction="row" justifyContent="flex-end" sx={{ mt: 3 }}>
              <Button
                ...
                disabled={isSubmitting || (saldoPendiente > 0 && Number(montoWatch) > saldoPendiente)}
              >
                {isSubmitting ? t('pago_form.guardando') : t('pago_form.guardar')}
              </Button>
            </Stack>
          </>
        )}
