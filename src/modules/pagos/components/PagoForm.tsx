...
function toFormDefaults(initialData?: Pago | null, presupuestoIdFijo?: number, trabajadorIdFijo?: number): PagoFormValues {
  return {
...
export function PagoForm({ initialData, presupuestoIdFijo, trabajadorIdFijo, onSubmit, isSubmitting = false }: Props) {
  const { control, handleSubmit, reset, setValue, setError, clearErrors, formState: { errors } } = useForm<PagoSchemaValues>({
    resolver: zodResolver(pagoSchema),
    defaultValues: toFormDefaults(initialData, presupuestoIdFijo, trabajadorIdFijo),
  });
  ...
  const presupuestoCancelado = Number(trabajadorIdWatch) > 0 && !isLoadingHistorial && saldoPendiente === 0 && historial.length > 0;

  // Validación dinámica del monto vs saldo pendiente
  useEffect(() => {
  ...
  useEffect(() => {
    reset(toFormDefaults(initialData, presupuestoIdFijo, trabajadorIdFijo));
    ...
    <Paper sx={{ p: 3, borderRadius: 3 }}>
      <Box component="form" onSubmit={handleSubmit(handleFormSubmit)}>
        ...
        <Typography variant="caption" sx={{ color: '#94A3B8', display: 'block', mb: 2 }}>
          El sistema completará automáticamente el trabajador, labor, equipo y cliente asociados.
        </Typography>

        <Grid container spacing={2} sx={{ mb: 3 }}>
          <Grid size={{ xs: 12 }}>
            <Controller name="presupuesto_id" control={control} render={({ field }) => (
              <TextField select fullWidth label="Presupuesto *" value={field.value}
                onChange={(e) => field.onChange(e.target.value === '' ? '' : Number(e.target.value))}
                disabled={!!presupuestoIdFijo}>
                <MenuItem value="">Seleccionar presupuesto</MenuItem>
                {presupuestos.map((p) => <MenuItem key={p.id} value={p.id}>{p.nombre || `Presupuesto #${p.id}`}</MenuItem>)}
              </TextField>
            )} />
          </Grid>
        </Grid>
                  ...
                  <Grid size={{ xs: 12, md: contexto.cliente ? 6 : 12 }}>
                    <Box sx={{ p: 2, borderRadius: 2, bgcolor: '#F8FAFC', border: '1px solid #E2E8F0', height: '100%' }}>
                      ...
                      <Stack direction="row" alignItems="center" justifyContent="space-between">
                        ...
                        <Chip label={contexto.labor.estado_nombre ?? 'Sin estado'} size="small"
            ...
            {isLoadingHistorial ? (
              ...
              <>
                ...
                <Grid container spacing={2} sx={{ mb: 2 }}>
                  ...
                  <Grid size={{ xs: 6, md: 3 }}>
                    <Box sx={{ p: 1.5, borderRadius: 2, bgcolor: saldoPendiente === 0 ? '#F1F5F9' : '#FEE2E2', textAlign: 'center' }}>
                ...
                {historial.length === 0 ? (
                  ...
                  <Stack spacing={1} sx={{ maxHeight: 240, overflowY: 'auto' }}>
                    {historial.map((p) => (
                      <Box key={p.id} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', p: 1.5, borderRadius: 2, bgcolor: '#FFFFFF', border: '1px solid #E2E8F0' }}>
                        <Box>
        ...
        {!presupuestoCancelado && Number(presupuestoIdWatch) > 0 && (
          <>
            ...
            {saldoPendiente > 0 && (
              <Typography variant="caption" sx={{ color: '#94A3B8', display: 'block', mb: 2 }}>
                Saldo disponible: <strong style={{ color: '#DC2626' }}>${saldoPendiente.toLocaleString('es-AR')}</strong>
              </Typography>
            )}
            ...
            <Grid container spacing={2}>
              <Grid size={{ xs: 12, md: 4 }}>
                <Controller name="monto" control={control} render={({ field }) => (
                  <TextField
                    ...
                    label="Monto *"
                    onChange={(e) => field.onChange(e.target.value === '' ? '' : Number(e.target.value))}
                    inputProps={{ max: saldoPendiente > 0 ? saldoPendiente : undefined }}
                    InputProps={{ startAdornment: <InputAdornment position="start">$</InputAdornment> }}
                    error={!!errors.monto}
                    helperText={errors.monto?.message ?? (saldoPendiente > 0 ? `Máximo: $${saldoPendiente.toLocaleString('es-AR')}` : '')}
                  />
                )} />
              </Grid>
              ...
              <Grid size={{ xs: 12, md: 4 }}>
                <Controller name="fecha" control={control} render={({ field }) => (
                  <TextField {...field} fullWidth type="date" label="Fecha *" InputLabelProps={{ shrink: true }}
              ...
              <Grid size={{ xs: 12, md: 4 }}>
                <Controller name="estado" control={control} render={({ field }) => (
                  <TextField select fullWidth label="Estado" value={field.value} onChange={field.onChange}>
              ...
              <Grid size={{ xs: 12, md: 6 }}>
                <Controller name="forma_pago_id" control={control} render={({ field }) => (
                  <TextField select fullWidth label="Forma de pago" value={field.value}
                    onChange={(e) => field.onChange(e.target.value === '' ? '' : Number(e.target.value))}>
