...
const COLORS = ['#F59E0B', '#2563EB', '#10B981', '#0F172A', '#EF4444'];
...
export function PagosEstadisticas() {
  ...
  const datosMes = por_mes.map((m) => ({ name: m.mes_label, total: Number(m.total) }));
  const datosTrabajador = por_trabajador.map((t) => ({
    name: t.trabajador.split(' ')[0],
    fullName: t.trabajador,
    total: Number(t.total_cobrado),
  }));
  const datosFormaPago = por_forma_pago.map((f) => ({ name: f.forma_pago, value: Number(f.total) }));
  ...
  return (
    <Box sx={{ mb: 3 }}>
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        <Grid size={{ xs: 6, md: 3 }}>
          <KPICard label={t('pagos_stats.total_general')} value={formatMonto(totalGeneral)} />
        </Grid>
        <Grid size={{ xs: 6, md: 3 }}>
          <KPICard label={t('pagos_stats.total_pagado')} value={formatMonto(totalPagado)} color="#15803D" />
        </Grid>
        <Grid size={{ xs: 6, md: 3 }}>
          <KPICard label={t('pagos_stats.total_parcial')} value={formatMonto(totalParcial)} color="#A16207" />
        </Grid>
        <Grid size={{ xs: 6, md: 3 }}>
          <KPICard label={t('pagos_stats.cantidad_pagos')} value={cantidadPagos.toString()} />
        </Grid>
      </Grid>
          ...
          <Card sx={{ borderRadius: 3, bgcolor: 'background.paper', border: `1px solid ${theme.palette.divider}`, boxShadow: 'none' }}>
            <CardContent sx={{ p: 3 }}>
              <Typography variant="body2" fontWeight={700} sx={{ color: 'text.secondary', mb: 2 }}>
                {t('pagos_stats.evolucion_mensual')}
              </Typography>
              <ResponsiveContainer width="100%" height={220}>
                <LineChart data={datosMes}>
                  <CartesianGrid strokeDasharray="3 3" stroke={theme.palette.divider} />
                  <XAxis dataKey="name" tick={{ fontSize: 12, fill: theme.palette.text.disabled }} />
                  <YAxis tick={{ fontSize: 12, fill: theme.palette.text.disabled }} tickFormatter={(v) => `$${(v / 1000).toFixed(0)}k`} />
                  <Tooltip content={<CustomTooltip />} />
                  <Line type="monotone" dataKey="total" stroke="#F59E0B" strokeWidth={2.5} dot={{ fill: '#F59E0B', r: 4 }} />
                </LineChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
            ...
            <CardContent sx={{ p: 3 }}>
              ...
              <ResponsiveContainer width="100%" height={180}>
                <PieChart>
                  ...
                  <Tooltip formatter={(v: any) => formatMonto(v)} />
                  <Legend formatter={(v) => <span style={{ fontSize: 12, color: theme.palette.text.secondary }}>{v}</span>} />
                </PieChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
      ...
      <Grid container spacing={2}>
        <Grid size={{ xs: 12 }}>
          <Card sx={{ borderRadius: 3, bgcolor: 'background.paper', border: `1px solid ${theme.palette.divider}`, boxShadow: 'none' }}>
            <CardContent sx={{ p: 3 }}>
              <Typography variant="body2" fontWeight={700} sx={{ color: 'text.secondary', mb: 2 }}>
                {t('pagos_stats.por_trabajador')}
              </Typography>
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={datosTrabajador} layout="vertical">
                  <CartesianGrid strokeDasharray="3 3" stroke={theme.palette.divider} horizontal={false} />
                  <XAxis type="number" tick={{ fontSize: 12, fill: theme.palette.text.disabled }} tickFormatter={(v) => `$${(v / 1000).toFixed(0)}k`} />
                  <YAxis type="category" dataKey="fullName" tick={{ fontSize: 12, fill: theme.palette.text.secondary }} width={120} />
                  <Tooltip content={<CustomTooltip />} />
