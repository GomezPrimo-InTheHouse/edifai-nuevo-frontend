...
export const PagoDetailPage: React.FC = () => {
  ...
  if (isLoading) return <LoadingState message={t('pagos.detail.loading')} />;
  if (isError) return <ErrorState title="Error" message={t('pagos.detail.error')} onRetry={refetch} />;
  if (!pago) return <ErrorState title={t('pagos.detail.no_encontrado')} message={t('pagos.detail.no_encontrado_msg')} />;

  const handleCambiarEstado = async (estado: string) => {
    try {
      await cambiarEstadoMutation.mutateAsync({ id: pagoId, estado });
      notify.success(t('pagos.detail.estado_actualizado'));
    } catch {
      notify.error(t('pagos.detail.error_estado'));
    }
  };
  ...
  const handleExportPDF = () => {
    ...
    ventana.document.write(`
      <html>
        <head>
          <title>${t('pagos.detail.comprobante_titulo')} #${pago.id}</title>
          <style>
            body { font-family: Arial, sans-serif; padding: 40px; color: #0F172A; }
            h1 { font-size: 24px; margin-bottom: 4px; }
            p { color: #64748B; margin: 0 0 24px; }
  ...
  return (
    <AppLayout>
      <PageHeader
        title={`${t('pagos.detail.comprobante_titulo')} #${pago.id}`}
        subtitle={t('pagos.detail.subtitle')}
        actions={
          <Stack direction="row" spacing={1}>
            <Button variant="outlined" startIcon={<ArrowLeft size={16} />} onClick={() => navigate('/pagos')}>
              {t('pagos.detail.volver')}
            </Button>
            <Button variant="contained" startIcon={<Download size={16} />} onClick={handleExportPDF}>
              {t('pagos.detail.exportar_pdf')}
            </Button>
          </Stack>
        }
      />
      ...
      <Grid container spacing={3}>
            ...
            <CardContent sx={{ p: 3 }}>
              <div ref={printRef}>
                <Typography variant="h5" fontWeight={800} sx={{ mb: 0.5 }}>
                  EdifAI — {t('pagos.detail.comprobante_titulo')}
                </Typography>
                <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
                  #{pago.id} — {new Date(pago.fecha).toLocaleDateString('es-AR')}
                </Typography>
                <Divider sx={{ mb: 3 }} />
                <table style={{ width: '100%' }}>
                  <tbody>
                    <tr>
                      <td style={{ color: theme.palette.text.secondary, padding: '8px 0', borderBottom: `1px solid ${theme.palette.divider}` }}>
                        {t('pagos.detail.trabajador')}
                      </td>
                      <td style={{ textAlign: 'right', fontWeight: 600, padding: '8px 0', borderBottom: `1px solid ${theme.palette.divider}`, color: theme.palette.text.primary }}>
                        {pago.trabajador_nombre} {pago.trabajador_apellido}
                      </td>
                    </tr>
                    <tr>
                      <td style={{ color: theme.palette.text.secondary, padding: '8px 0', borderBottom: `1px solid ${theme.palette.divider}` }}>
                        {t('pagos.detail.presupuesto')}
                      </td>
                      <td style={{ textAlign: 'right', fontWeight: 600, padding: '8px 0', borderBottom: `1px solid ${theme.palette.divider}`, color: theme.palette.text.primary }}>
                        {pago.presupuesto_nombre || `#${pago.presupuesto_id}`}
                      </td>
                    </tr>
                    <tr>
                      <td style={{ color: theme.palette.text.secondary, padding: '8px 0', borderBottom: `1px solid ${theme.palette.divider}` }}>
                        {t('pagos.detail.forma_pago')}
                      </td>
                      <td style={{ textAlign: 'right', fontWeight: 600, padding: '8px 0', borderBottom: `1px solid ${theme.palette.divider}`, color: theme.palette.text.primary }}>
                    ...
                    <tr>
                      <td style={{ padding: '16px 0', fontSize: 18, fontWeight: 800, color: theme.palette.text.primary }}>
                        {t('pagos.detail.total')}
                      </td>
                      <td style={{ textAlign: 'right', fontSize: 24, fontWeight: 800, padding: '16px 0', color: theme.palette.text.primary }}>
                        ${Number(pago.monto).toLocaleString('es-AR')}
                      </td>
                    </tr>
                  </tbody>
        ...
        <Grid size={{ xs: 12, md: 4 }}>
          <Card sx={{ borderRadius: 3, bgcolor: 'background.paper', border: `1px solid ${theme.palette.divider}`, boxShadow: 'none' }}>
            <CardContent sx={{ p: 3 }}>
              <Typography variant="h6" fontWeight={700} sx={{ mb: 2 }}>{t('pagos.detail.estado_pago')}</Typography>
              <Divider sx={{ mb: 3 }} />
              <PagoEstadoChip estado={pago.estado} />
              <Box sx={{ mt: 3 }}>
                <Typography variant="body2" fontWeight={600} sx={{ mb: 1, color: 'text.secondary' }}>
                  {t('pagos.detail.cambiar_estado')}
                </Typography>
                <TextField
                  select fullWidth size="small"
                  value={pago.estado}
